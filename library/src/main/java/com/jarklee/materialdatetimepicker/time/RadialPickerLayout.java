...
import com.jarklee.materialdatetimepicker.R;
...
public class RadialPickerLayout extends FrameLayout implements OnTouchListener {
    ...
    public void updateLocale() {
        if (mAmPmCirclesView != null) {
            mAmPmCirclesView.updateLocale();
        }
        invalidate();
    }
    ...
    public void initialize(Context context, TimePickerController timePickerController,
        ...
        for (int i = 0; i < 12; i++) {
            hoursTexts[i] = is24HourMode ?
                    String.format(getLocale(), "%02d", hours_24[i]) : String.format(getLocale(), "%d", hours[i]);
            innerHoursTexts[i] = String.format(getLocale(), "%d", hours[i]);
            minutesTexts[i] = String.format(getLocale(), "%02d", minutes[i]);
            secondsTexts[i] = String.format(getLocale(), "%02d", seconds[i]);
        }
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                if (mIsTouchingAmOrPm == AM || mIsTouchingAmOrPm == PM) {
                    ...
                    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                        forceLegal = mAccessibilityManager.isTouchExplorationEnabled();
                    }
